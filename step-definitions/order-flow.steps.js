...
Then('click on view weekly menu', async function () {
    ...
    try {
      ...
      if (count > 0) {
        try {
          ...
          console.log(`Error clicking with selector ${selector}:`, e.message);
        }
      }
    } catch (e) {
...
Then('the pickup location dropdown should be available', async function () {
...
Then('at least one location should be available in the dropdown', async function () {
...
When('I select a location from the pickup location dropdown', async function () {
  try {
    ...
    if (dropdownVisible) {
      try {
        ...
        try {
          ...
          console.log('Could not select location from dropdown:', e.message);
        }
      } catch (e) {
        console.log('Error clicking location dropdown:', e.message);
      }
    } else {
      console.log('Location dropdown not available, skipping location selection');
    }
  } catch (e) {
    console.log('Error in location selection step:', e.message);
  }
...
Then('the selected location should be displayed', async function () {
...
When('I select a pickup date from available dates', async function () {
    ...
    try {
      ...
      if (dateButtons.length > 0) {
        ...
        try {
          ...
          console.log(`Could not click date button: ${e.message}`);
        }
      }
    } catch (e) {
...
When('I check if menu is available for the selected date', async function () {
  // Wait for menu to load
  await this.page.waitForTimeout(1000);

  // Store the result for later use
  const mealCardsVisible = await this.menuPage.areMealCardsVisible();
  this.menuAvailable = mealCardsVisible;
});

When('the menu is not available, I select another available date', async function () {
...
When('I add a different meal to cart', async function () {
...
When('I add another different meal to cart', async function () {
...
Then('the cart should have at least {int} items', async function (expectedCount) {
...
Then('the cart should have at least {int} item', async function (expectedCount) {
...
Then('the cart total should be calculated correctly after item removal', async function () {
...
When('I enter an invalid promotion code {string} in the cart', async function (promoCode) {
...
Then('an error message should be displayed for invalid promotion code', async function () {
...
Then('all order details should be visible in checkout', async function () {
  await this.page.waitForTimeout(1500);
  ...
  try {
    ...
    console.log('Error checking order details:', e.message);
  }
...
Then('the order summary should show correct item count', async function () {
...
Then('the order summary should show correct total price', async function () {
