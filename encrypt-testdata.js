  ...
  if (testData.users) {
    Object.keys(testData.users).forEach(userType => {
      if (testData.users[userType].password) {
        const plainPassword = testData.users[userType].password;
        testData.users[userType].password = encryptionService.encrypt(plainPassword);
        console.log(`✓ Encrypted ${userType} user password`);
      }
    });
  }

  // Encrypt registration passwords
...
function decryptTestData() {
  ...
  if (testData.users) {
    Object.keys(testData.users).forEach(userType => {
      if (testData.users[userType].password) {
        try {
          const decrypted = encryptionService.getPlainPassword(testData.users[userType].password);
          testData.users[userType].password = decrypted;
  ...
  // Decrypt registration passwords
  if (testData.registration) {
    if (testData.registration.password) {
      try {
...
if (command === 'encrypt') {
  ...
  console.log('================================================\n');
  console.log('Usage:');
  console.log('  node encrypt-testdata.js encrypt   - Encrypt all passwords in testData.json');
  console.log('  node encrypt-testdata.js decrypt   - Decrypt all passwords in testData.json\n');
  console.log('Examples:');
  console.log('  node encrypt-testdata.js encrypt');
  console.log('  node encrypt-testdata.js decrypt\n');
